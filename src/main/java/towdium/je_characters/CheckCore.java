...
public class CheckCore {
    ...
    public static int[] checkRepresentation(Object rep, String str, int index) {
        if (rep instanceof Integer) {
            ...
            int remaining = str.length() - index;
            for (Object obj : strs) {
                String oneResult = (String) obj;
                if (oneResult.length() >= remaining && oneResult.startsWith(str.substring(index))) {
                    return new int[]{remaining};
                }
                if (str.startsWith(oneResult, index)) {
                    ret.add(oneResult.length());
                }
            }
